...
if (isset($_GET['category'])) {
    ...
    // Fetch products from the database based on the category and available products
    $query = "SELECT * FROM products WHERE category = '$category' AND available = 1";
    $result = $mysqli->query($query);
    $products = $result->fetch_all(MYSQLI_ASSOC);
} else {
    // If the category parameter is not provided, fetch products from all categories and available products
                        ...
                        <tbody id="cart-table-body">
                            <?php foreach ($cart as $index => $product) { ?>
                                <tr>
                                    ...
                                    <td>
                                        <button class="btn btn-sm btn-danger btn-remove-from-cart" data-index="<?php echo $index; ?>" data-product-id="<?php echo $product['product_id']; ?>">Remove</button>
                                    </td>
                                </tr>
                            <?php } ?>
                        </tbody>
         ...
         $(document).ready(function () {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response !== 'error') {
                            ...
                            // Parse the JSON response to get the updated cart data
                            var cartData = JSON.parse(response);

                            // Update the cart contents with the updated data
                            updateCartContents(cartData);
                        } else {
            ...
            $(document).ready(function() {
                $.ajax({
                    ...
                    type: "GET",
                    success: function(response) {
                        $('#transactions-table-body').html(response);
                    }
                });
            });
